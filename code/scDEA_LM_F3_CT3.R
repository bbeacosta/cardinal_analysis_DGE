...
base_dir <- "/home/ivm/CT3_DGE_analysis/"
dir.create(base_dir, showWarnings = FALSE)

################ Load all necessary files for analyses ##################
##### load Freeze 3 pseudobulk data for CT3 #####
# Create counts directories
dir_bangpaki <- "/genesandhealth/red/Beatrice_Costa/gh-qced-cells"
...
out_dir <- file.path(base_dir, "filtered_counts")
...
results <- data.frame(
  celltype = character(),
  donors_ge_10 = integer(),
  donors_after_intersection = integer(),
  status = character(),
...
for(cf in counts_files){
  ...
  if (!file.exists(ncells_file)) {
    results <- rbind(
      ...
      data.frame(
        ...
        donors_ge_10 = 0,
        donors_in_counts = NA,
        donors_after_intersection = 0,
        status = "missing_ncells"
      )
    )
    next
  }
  ...
  donors_in_counts <- ncol(counts_mat)
  rownames(counts_mat) <- gene_col
  colnames(counts_mat) <- colnames(counts)[-1]
  colnames(counts_mat) <- gsub("\\.\\.", "__", colnames(counts_mat))

  # 4. Read ncells, filter rows with N_cells >= 10
  ncells <- read.delim(ncells_file, header=TRUE, sep="\t", stringsAsFactors=FALSE)
  ncells_keep <- ncells %>%
    dplyr::filter(N_cells >= 10) # Either change threshold (to lower) or add a couple lines to skip this celltype if no donors are available

  donors_ge_10 <- nrow(ncells_keep)
  
  if (donors_ge_10 == 0) {
    message("Skipping celltype ", celltype, ": no donors with N_cells >= 10")
# 
# if (nrow(ncells_keep) == 0) {
#   message("Skipping celltype ", celltype,
#           ": no donors with N_cells >= 10")
#   next
# }
    ...
    results <- rbind(
      ...
      data.frame(
        celltype = celltype,
        donors_ge_10 = 0,
        donors_in_counts = donors_in_counts,
        donors_after_intersection = 0,
        status = "failed_N_cells"
      )
    )
    next
  }
  ...
  donors_after <- ncol(counts_filt)
  
  if (donors_after == 0) {
    message(
      "Skipping celltype ", celltype,
      ": no donors left after intersecting counts matrix"
    )
    ...
    results <- rbind(
      ...
      data.frame(
        celltype = celltype,
        donors_ge_10 = donors_ge_10,
        donors_in_counts = donors_in_counts,
        donors_after_intersection = 0,
        status = "failed_intersection"
      )
    )
    next
  }
#   
#   if (ncol(counts_filt) == 0) {
#     message("Skipping celltype ", celltype,
#             ": no donors left after intersecting with counts matrix")
#     next
#   }
#   
#   
  ...
  results <- rbind(
    ...
    data.frame(
      ...
      donors_ge_10 = donors_ge_10,
      donors_in_counts = donors_in_counts,
      donors_after_intersection = donors_after,
      status = "kept"
    )
  )
}
...
write.table(
  results,
  file = file.path(out_dir, "celltype_filtering_summary.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
...
# Check results - how many cell types and donors kept
table(results$status)
subset(results, status != "kept")
subset(results, status == "kept")

# merge(df, results, by = "celltype", all = TRUE)
# head(ncells_keep$unique.id)
 ...
 # disease_col <- "ATOPIC_DERM"
  ...
  for (disease_col in filtered_diseases) {
    ...
    if (qr(design_d)$rank < ncol(design_d)) {
      message("Skipping ", ct, " - ", disease_col,
              ": design matrix rank deficient (likely collinearity with BMI)")
      next
    }
    ...
    if (nrow(tt_d) == 0) {
      message("Skipping ", ct, " - ", disease_col,
              ": no valid coefficients returned by topTable()")
      next
    }
...
# 
# 
# # Tally of how many celltypes generated DGE results
# 
# # Count unique cell types that had at least one modeled disease
# dge_celltypes <- case_counts_df %>%
#   filter(cases >= min_cases) %>%  # ensures you only count diseases that passed the filter
#   distinct(celltype) %>%
#   nrow()
# 
# message(dge_celltypes, " cell types generated DGE results")
...
# f <- "/home/ivm/CT3_DGE_analysis//Monocyte_CD14_S100/data/Monocyte_CD14_S100_DGE_J10_PNEUMOBACT_top.rds"
...
# B_exhausted <- read.csv2(file = "/home/ivm/CT3_DGE_analysis/data/B_exhausted_ancestry_topTable.csv", header = TRUE, sep = ",")
# head(B_exhausted)
...
# remove_ct <- c("B_CD5", "Basophil", "HSC_MPP", "ILCP", "NK_prolif", 
#                "Plasmablast", "Platelet", "T_CD8_EMRA_IFN", "T_CD8_prolif")
# 
# deg_mat_prop <- deg_mat_prop[!rownames(deg_mat_prop) %in% remove_ct, drop = FALSE]
...
# # Remove empty celltype
# deg_mat_abs <- deg_mat_abs[rownames(deg_mat_abs) != "B_CD5", "Basophil", "HSC_MPP", "ILCP", "NK_prolif", 
#                            "Plasmablast", "Platelet", "T_CD8_EMRA_IFN", "T_CD8_prolif", ]
...
base_dir <- "/home/ivm/CT3_DGE_analysis_csv_annotated/"
